...
#se volessi re implementare lo switch case togli qui i commenti

#case ${waybar_cava_stbmode:-} in
#0)
#  stbBar='' # Clean: no display
#  ;;
#1)
#  stbBar=$(printf '%*s' "${bar_width}" "") # Blank: spaces matching bar width
#  ;;
#2)
#  stbBar=$(printf '%0.s${bar: -1}' $(seq 1 "${bar_width}")) # Full: full horizontal bar
#  ;;
#3)
stbBar=$(printf '%0.s0' $(seq 1 "${bar_width}")) # Low: horizontal line with the lowest bar character
#  ;;
#*)
#  stbBar="${waybar_cava_stbmode}" # Custom string
#  ;;
#esac
